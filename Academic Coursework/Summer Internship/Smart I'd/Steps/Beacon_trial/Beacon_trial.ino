...
const float measurementNoise = 10.0; // Measurement noise variance (adjust as needed)
const float processNoise = 0.01; // Process noise variance (adjust as needed)
KalmanFilter rssiFilter(measurementNoise, processNoise);
  ...
  if (client.connect(centralSystemAddress, port)) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (device.haveName() && device.getName() == "BT05") {
        ...
        // Filter the RSSI measurement using the Kalman filter
        float filteredRssi = rssiFilter.update(rawRssi, 0); // Passing 0 as newRate

        String data = "( 1, " + address + ", " + String(filteredRssi) + " )";
        client.println(data);
